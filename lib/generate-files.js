...
const gitOptions = { checkoutBranch: 'develop' }
...
class GenerateManager {
  ...
  async clone() {
    ...
    await Promise.all(this.nodes.map(node => {
      ...
      return exec(`git clone ${gitUrl} ${nodePath} --branch ${gitOptions.checkoutBranch} --single-branch`)
    }))
  ...
  async createFiles() {
    ...
    const thisCryptoNetworkPath = path.join(thisNetworkPath, 'crypto')
    const thisCustomNetworkPath = path.join(thisCryptoNetworkPath, this.network)
    ...
    for (const [index, node] of this.nodes.entries()) {
      ...
      const distCoreNetworkPath = path.join(distNodePath, 'packages/core/src/config/e2enet')
      const distCryptoNetworkPath = path.join(distNodePath, 'packages/crypto/src/networks')
      const distCryptoCustomNetworkPath = path.join(distCryptoNetworkPath, this.network)
      const distDockerPath = path.join(distNodePath, 'docker/testnet') //TODO create our own folder

      await exec(`mkdir ${distCoreNetworkPath}`)
      await exec(`mkdir ${distCryptoCustomNetworkPath}`)
      ...
      copyFiles([
        ...
        {
          from: thisCryptoNetworkPath,
          to: distCryptoNetworkPath,
          files: [ 'index.ts' ]
        },
        {
          from: thisCustomNetworkPath,
          to: distCryptoCustomNetworkPath,
          files: [ 'index.ts', 'network.json', 'milestones.json', 'exceptions.json' ]
        },
        {
          from: thisDockerPath,
          to: distDockerPath,
