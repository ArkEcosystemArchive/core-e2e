...
class TestRunner {
  ...
  async launchNodes() {
    for (let nodeNumber in this.nodes) {
      ...
      const commandPeerIP = `sed -i 's/<nodeIpPlaceholder>/${IPPreviousNode}/g' ${this.rootPath}/dist/${this.network}/${nodeInfos.name}/packages/core/src/config/e2enet/peers.json`
