...
class Helpers {
  async GET(path, params = {}, port = 4300) {
    return this.request('GET', path, params, port)
  }

  async POST(path, params = {}, port = 4300) {
    return this.request('POST', path, params, port)
  }

  async request (method, path, params = {}, port = 4300) {
    const url = `http://127.0.0.1:${port}/api/v2/${path}`
